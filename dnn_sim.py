...
GPU = 5
class QueryDNNSim(object):
    def __init__(self, query_len, title_len, num_word, embed_size, hidden_size1, hidden_size2, load_path, gpu=GPU, max_grad_norm=5, margin=0.1):
        ...
        with tf.device("/gpu:%s" % (gpu)):
            ...
            with tf.name_scope('join_layer'):
                pos_concat = tf.concat(1, [self.query_vec, pos_match, self.pos_title_vec])
                neg_concat = tf.concat(1, [self.query_vec, neg_match, neg_title_vec])

            with tf.variable_scope('hidden1'):
                initializer = tf.contrib.layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32)
                w1 = tf.get_variable('w1', [embed_size * 2 + 1, hidden_size1], initializer=initializer) * 0.1
...
def train(fn_train, fn_feature, fn_word,
          ...
          embed_size, hidden_size1, hidden_size2, load, method, test_conf, n_repeat=1, margin=0.1):
    ...
    model = QueryDNNSim(MAX_QUERY_LEN, MAX_TITLE_LEN, num_word, embed_size, hidden_size1, hidden_size2, load_path, margin=margin)
    ...
    for epoch_index in xrange(num_epoch):
        ...
        for i in xrange(n_batch):
            ...
            queries, pos_titles, neg_titles, margins = dataset.next_batch(batch_size, n_repeat)
...
def dnn_click_clean():
    ...
    train(fn_train, fn_feature, fn_word,
          ...
          embed_size=100, hidden_size1=100, hidden_size2=50, load=False, method=2,
...
def dnn_click():
    ...
    # train(fn_train, fn_feature, fn_word,
    #       num_epoch=500, batch_size=300, dirname='dnn-click-sim',
    #       embed_size=100, hidden_size1=100, hidden_size2=50, load=True, method=2, gpu=5,
    #       test_conf={'fn_test': fn_dev, 'fn_title': fn_dev_title,
    #                  'test_cls': 'CombinedTestData'})

    # train(fn_train, fn_feature, fn_word,
    #       num_epoch=500, batch_size=300, dirname='dnn-click-sim-repeat2-margin0.1',
    #       embed_size=100, hidden_size1=100, hidden_size2=50, load=False, method=2, gpu=5,
    #       test_conf={'fn_test': fn_dev, 'fn_title': fn_dev_title,
    #                  'test_cls': 'CombinedTestData'}, n_repeat=2)

    # train(fn_train, fn_feature, fn_word,
    #       num_epoch=500, batch_size=300, dirname='dnn-click-sim-repeat3-margin0.1',
    #       embed_size=100, hidden_size1=100, hidden_size2=50, load=False, method=2,
    #       test_conf={'fn_test': fn_dev, 'fn_title': fn_dev_title,
    #                  'test_cls': 'CombinedTestData'}, n_repeat=3)

    # train(fn_train, fn_feature, fn_word,
    #       num_epoch=500, batch_size=300, dirname='dnn-click-sim-repeat1-margin0.2',
    #       embed_size=100, hidden_size1=100, hidden_size2=50, load=False, method=2,
    #       test_conf={'fn_test': fn_dev, 'fn_title': fn_dev_title,
    #                  'test_cls': 'CombinedTestData'}, n_repeat=3, margin=0.2)

    train('data/com.query.click.20150924.20160630.v', 'data/title.20150924.20160630.v', fn_word,
          num_epoch=500, batch_size=300, dirname='dnn-click-sim-large',
          embed_size=100, hidden_size1=100, hidden_size2=50, load=False, method=2,
          test_conf={'fn_test': fn_dev, 'fn_title': fn_dev_title,
                     'test_cls': 'CombinedTestData'}, n_repeat=1, margin=0.1)

    # train(fn_train, fn_feature, fn_word,
    #       num_epoch=500, batch_size=500, dirname='dnn-click-sim-constant',
    #       embed_size=100, hidden_size1=100, hidden_size2=100, load=True, method=2, gpu=3,
    #       test_conf={'fn_test': fn_dev, 'fn_title': fn_dev_title,
    #                  'test_cls': 'CombinedTestData'})
